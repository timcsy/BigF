...
const int precise=100;
class Big
{
	friend int max(Big a,Big b)
	{if(a[-2]>=b[-2]) return a[-2];else return b[-2];}
	friend void align(Big& a,Big& b)
	{int n=a[-3]-b[-3];bool m=n>=0;if(!m)n=-n;for(int i=0;i<n;i++){(m?a:b).shift(0);(m?a:b)[-3]++;}}
	...
	private:
		int block[blockLength]; //[i] 
		bool sign; //(1) 
		int digit;//the max index now //[-2]
		int point; //[-3] //[-4]is digit-point means integer part
	public:
		Big();
		Big(int n);
		...
		Big operator--(int){Big t=*this;--*this;return t;}
		int& operator [](int i)
		{if(i==-4){int* j=new int(digit-point);return *j;}if(i==-3)return point;if(i==-2)return digit;else return block[i];}
		bool& operator ()(int i)
		{if(i==1)return sign;}
		template<class T>
...
Big operator/(Big a,Big b)
{
	...
	else
	{
		...
		{
			c[-2]=a[-2]-b[-2]+2+precise;
			if(a[-3]>=b[-3]) c[-3]=a[-3]-b[-3]+precise;
			else{for(int i=0;i<b[-3]-a[-3];i++){a.shift(0);a[-3]++;}c[-3]=precise;}
			Big t(a,a[-2]-b[-2],a[-2]);
			for(int i=a[-2]-b[-2];i>0;i--)
			{
				int q=a.decide(t,b);
				c[i+precise]=q; t-=b*q;
				t.shift(a[i-1+precise]);
			}
			c[precise]=a.decide(t,b);
